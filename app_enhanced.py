...
try:
    ...
    st.error(
        "❌ Reality Defender SDK not installed. Please install it with: pip install realitydefender"
    )
    st.stop()
...
try:
    ...
    st.error(
        "❌ API key not found. Please add REALITY_DEFENDER_API_KEY to your Streamlit secrets."
    )
    st.stop()
...
def display_results_enhanced(result: Dict[Any, Any]) -> None:
    ...
    # 🤖 MODEL CONSENSUS with visual indicators
...
# ───────────────────────── STREAMLIT UI
st.set_page_config(page_title="Reality Defender - Deepfake Detection",
                   page_icon="🕵️",
                   layout="centered")
...
<div style='text-align: center; padding: 2rem; background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); border-radius: 10px; margin-bottom: 2rem;'>
    <h1 style='color: white; margin: 0; font-size: 2.5rem;'>🕵️‍♀️ Reality Defender</h1>
...
st.caption(
    "Powered by Reality Defender Python SDK | v3.0 - Auto-discovery methods")
